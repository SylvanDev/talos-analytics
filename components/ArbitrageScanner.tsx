...
import { RefreshCw, Zap, Database, DollarSign, Calculator } from 'lucide-react';
...
export const ArbitrageScanner: React.FC = () => {
  ...
  const runScan = async () => {
    ...
    try {
        ...
        // 2. Find Arbs
        const arbs = await findArbitrageOpportunities(polyEvents, useMock);
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4 bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
            <div>
                ...
                <p className="text-slate-400 text-sm mt-2 max-w-xl">
                    Hedge Polymarket positions against traditional bookmakers. 
                    Calculates perfect sizing for guaranteed profit.
                </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-end sm:items-center gap-4 w-full lg:w-auto">
                ...
                <div className="flex items-center gap-2 bg-slate-950 rounded-lg p-1 border border-slate-800">
                    <button 
                        ...
                        className={`px-3 py-2 text-xs font-bold rounded transition-colors ${useMock ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                        TEST DATA
                    </button>
                    <button 
                        onClick={() => setUseMock(false)}
                        className={`px-3 py-2 text-xs font-bold rounded transition-colors ${!useMock ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                        LIVE API
                    </button>
                </div>
        ...
        {opportunities.length === 0 && !loading ? (
            <div className="flex-grow flex flex-col items-center justify-center text-slate-500 border-2 border-dashed border-slate-800 rounded-2xl p-12">
                ...
                <p className="text-sm mt-2 max-w-md text-center">
                    Markets are currently efficient or no matching events were found between Polymarket and Bookmakers. Try switching to "Test Data" to see how it works.
                </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {opportunities.map((arb) => {
                    // DYNAMIC CALCULATION BASED ON USER BANKROLL
                        ...
                        <div key={arb.id} className="bg-[#1e293b]/50 border border-slate-700 rounded-xl overflow-hidden hover:border-blue-500/50 transition-colors group flex flex-col">
                            ...
                            <div className="bg-slate-900/80 p-4 border-b border-slate-700 flex justify-between items-center">
                                <div className="flex items-center gap-2">
                                    <span className="bg-emerald-900/30 text-emerald-400 border border-emerald-900/50 px-2 py-1 rounded text-xs font-bold font-mono">
                                        +{(arb.profitMargin * 100).toFixed(2)}% ROI
                                    </span>
                                    <span className="text-xs text-slate-500 font-mono">
                                        Conf: {(arb.confidence * 100).toFixed(0)}%
                                    </span>
                                </div>
                                <div className="text-emerald-400 font-bold text-sm flex items-center gap-1 bg-emerald-950/50 px-2 py-1 rounded border border-emerald-900/30">
                            ...
                            <div className="p-5 space-y-6 flex-grow">
                                ...
                                {/* Side B: Bookmaker */}
                                <div className="relative">
                                    <div className="absolute -left-5 top-1/2 -translate-y-1/2 w-1 h-8 bg-orange-500 rounded-r"></div>
                                    <div className="text-[10px] text-slate-500 uppercase font-bold mb-1 flex items-center gap-1">
                                        {arb.bookmakerName} (Hedge)
                                    </div>
                                    <div className="flex justify-between items-start">
                                        <div className="pr-2">
                                            <div className="text-white font-bold text-sm line-clamp-1" title={arb.bookmakerEvent}>{arb.bookmakerEvent}</div>
                                            <div className="text-orange-400 text-xs mt-0.5 font-bold">{arb.bookmakerTeam}</div>
                                        </div>
                                        <div className="text-right shrink-0">
                            ...
                            <div className="bg-slate-950 p-4 border-t border-slate-800">
                                <div className="flex justify-between text-xs mb-3 font-mono">
                                    <span className="text-slate-400">Total Wager: <span className="text-slate-200">${bankroll.toFixed(0)}</span></span>
                                    <span className="text-emerald-400">Net: ${(bankroll + profit).toFixed(2)}</span>
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="flex flex-col gap-1">
                                        <a 
                                            href={arb.polymarketUrl} target="_blank" rel="noreferrer"
                                            className="bg-blue-600 hover:bg-blue-500 text-white border border-blue-500 rounded py-2 text-center text-xs font-bold transition-colors shadow-lg shadow-blue-900/20"
                                        >
                                            Bet ${stake1.toFixed(2)}
                                        </a>
                                        <span className="text-[10px] text-center text-slate-500">on Polymarket</span>
                                    </div>
                                    <div className="flex flex-col gap-1">
                                        <a 
                                            href={arb.bookmakerUrl} target="_blank" rel="noreferrer"
                                            className="bg-orange-600 hover:bg-orange-500 text-white border border-orange-500 rounded py-2 text-center text-xs font-bold transition-colors shadow-lg shadow-orange-900/20"
                                        >
                                            Bet ${stake2.toFixed(2)}
                                        </a>
                                        <span className="text-[10px] text-center text-slate-500">on {arb.bookmakerName}</span>
                                    </div>
                                </div>
                            </div>
